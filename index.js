import mRNA from './mRNA.json' assert { type: "json" };
...
const sequence = (YOUR_SEQUENCE).replaceAll('A', 'U').replaceAll('T', 'A').replaceAll('C', 'H').replaceAll('G', 'C').replaceAll('H', 'G') + (YOUR_SEQUENCE.charAt(-1) != ' ' ? ' ' : '');
let tempString = sequence;
let final = '';
console.log('Before the process starts, please make sure that you\'ve inputted your sequence in the YOUR_SEQUENCE variable. To begin, input 1 and press enter. If the process is taking over 15 seconds, please stop the process and check your sequence. (Only put DNA sequences in the variable, NOT mRNA, and make sure there is a space between each letter.)');
if (prompt('=') == 1) {
  let forceBreak = false;
  setTimeout(() => {
    if (!forceBreak) {
      forceBreak = true;
      console.log('Process taking longer than 15 seconds. Please check your sequence for errors.');
    }
  }, 15000);
  while (!forceBreak) {
    ...
    let preSequence = '';
    for (let i = tempString.search('A U G') + 1; i--;) preSequence += tempString.charAt(i - 1);
    tempString = tempString.replace(preSequence.split('').reverse().join(''), '');
    final += preSequence.split('').reverse().join('') + '- ';
    let regex = /([A-Z] [A-Z] [A-Z] )/g,
      list = tempString.split(regex);
    for (let i = list.length; i--;) '' === list[i] && list.splice(list.indexOf(''), 1);
    let e = '';
    list.every((l) => {
      e += ('A U G ' == l ? (!e.includes('(START)') ? '(START) ' : '') + l : 'U A A ' == l || 'U A G ' == l || 'U G A ' == l ? l + '(STOP) ' : l) + '- ';
      if (e.includes('(STOP)')) {
  ...
  if (final.match(/(START)/g).length != final.match(/(STOP)/g).length) {
    ...
    proteins.forEach((i) => {
      aminoAcids[index] = i.split(' (STOP) ')[0].replaceAll(' ', '').split('-');
      index++;
    });
    aminoAcids.forEach((i) => {
      let codonsIndex = 0;
      i.forEach((e) => {
        i[codonsIndex] = mRNA[e.charAt(0)][e.charAt(1)][e.charAt(2)];
        codonsIndex++;
      });
    });
    while (1) {
      ...
      const protein = prompt('=') - 1;
      console.log('');
      console.log('Please enter the index of the amino acid. (0 is last, based on array index - 1)');
      const aminoAcid = prompt('=') - 1;
      console.clear();
      console.log('=> Amino Acid #' + (aminoAcid + 1) + ' for protein #' + (protein + 1) + ': ' + aminoAcids[protein].at(aminoAcid));
    }
  }
} else {
